...
def extract_frames(video_path, num_frames=30, start_frame=0):
    """
    ...
    Args:
        ...
        start_frame: Frame index to start sampling from (for alignment).
    ...
    # Apply start offset (clamped)
    start = max(0, min(start_frame, total - 1))
    effective_total = total - start

    if effective_total <= 0:
        black = np.zeros((h or 540, w or 960, 3), dtype=np.uint8)
        return [(0, black.copy()) for _ in range(num_frames)]

    # Uniform sampling from start to end
    if num_frames >= effective_total:
        indices = [start + i for i in range(effective_total)]
    else:
        step = (effective_total - 1) / (num_frames - 1) if num_frames > 1 else 0
        indices = [start + int(round(i * step)) for i in range(num_frames)]

    sampled = [(idx, all_frames[idx]) for idx in indices]

    duration = total / fps if fps > 0 else 0
    offset_msg = f", offset={start}" if start > 0 else ""
    print(f"  {video_path}: {total} frames, {duration:.1f}s @ {fps:.0f}fps"
          f"{offset_msg} -> sampled {len(sampled)} frames")
    return sampled
...
def process_video_pair(teacher_path, student_path, num_frames=30,
                       student_offset=0):
    """
    Sample frames across the ENTIRE video duration, run center-person pose
    detection on each sampled frame.

    Args:
        teacher_path: Path to teacher video.
        student_path: Path to student video.
        num_frames:   Frames to sample from each video.
        student_offset: Frame offset for student (shifts timeline for alignment).
...
def generate_comparison_video(processed_data, output_path, fps=5):
    ...
    # Use H.264 for browser compatibility (Gradio/web playback)
